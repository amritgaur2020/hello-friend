...
import { useHousekeepingInventory, useHousekeepingMutations } from '@/hooks/useDepartmentData';
...
import { HOUSEKEEPING_UNITS } from '@/constants/inventoryUnits';
...
export default function HousekeepingInventory() {
  ...
  const { addInventory, updateInventory, deleteInventory, bulkAddInventory, addTransaction } = useHousekeepingMutations();
  const { settings } = useHotelSettings();
  const { isAdmin, hasPermission } = useAuth();
  const currencySymbol = settings?.currency_symbol || '₹';
  
  const canEdit = isAdmin || hasPermission('housekeeping', 'edit');
  const canDelete = isAdmin || hasPermission('housekeeping', 'delete');
  ...
  const [newItem, setNewItem] = useState({ name: '', category: 'cleaning', unit: 'pcs', current_stock: 0, min_stock_level: 5, cost_price: 0 });

  const handleAddItem = async () => {
    await addInventory.mutateAsync(newItem);
    setShowAddDialog(false);
    setNewItem({ name: '', category: 'cleaning', unit: 'pcs', current_stock: 0, min_stock_level: 5, cost_price: 0 });
  };
  ...
  return (
    <DashboardLayout title="Housekeeping Inventory" subtitle="Manage cleaning supplies and amenities">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <InventoryExcelImport departmentName="Housekeeping" categories={CATEGORIES} units={HOUSEKEEPING_UNITS} onImport={handleBulkImport} isLoading={bulkAddInventory.isPending} />
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2"><Label>Unit</Label>
                <Select value={newItem.unit} onValueChange={v => setNewItem({ ...newItem, unit: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{HOUSEKEEPING_UNITS.map(u => <SelectItem key={u.value} value={u.value}>{u.label}</SelectItem>)}</SelectContent>
                </Select>
              </div>
      ...
      <InventoryEditDialog open={!!editItem} onOpenChange={(open) => !open && setEditItem(null)} item={editItem} categories={CATEGORIES} units={HOUSEKEEPING_UNITS} onSave={handleEditSave} isLoading={updateInventory.isPending} />
